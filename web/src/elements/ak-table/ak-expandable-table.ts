...
const ExpandableTableBase = ExpansionRenderer(SimpleTable);

/**
 * @element ak-expandable-table
 * class Table
 *
 * Extends the SimpleTable so that an expandable row can be shown below a content row.
 *
 * Because both this implementation and `ak-full-table` use the exact same rendering infrastructure,
 * that has been broken out into a mixin named [`ExpandableRenderer`](./ExpandableRenderer.ts).
 *
 * ## Properties
 *
 ...
 *   include the expandable content as part of the payload.
 *
 ...
 * - @part expand-cell: The `<td>` of the button that controls expandable
 * - @part expand-icon: The `<div>` that contains the expandable icon
...
export class ExpandableTable extends ExpandableTableBase implements ISimpleTable {
    ...
    // This override adds room for the expandable control.
